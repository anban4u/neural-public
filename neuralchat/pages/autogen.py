...
class BoardAgent(autogen.AssistantAgent):
    ...
    def _generate_board_reply(
        ...
        message = messages[-1]
        st.write(sender.name + ":" + message['content'])
        ...
        try:
            ...
            st.image(chess.svg.board(self.board, arrows=[(m.from_square, m.to_square)], fill={m.from_square: "gray"}, size=200))
...
class ChessPlayerAgent(autogen.AssistantAgent):
    ...
    def _generate_reply_for_board(
        ...
        st.write(sender.name + ":" + last_message['content'])
